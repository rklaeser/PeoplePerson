...
import { Region, Intent } from '$lib/db/models/Person';
...
export async function POST({ request }) {
  try {
    ...
    const prompt = PromptTemplate.fromTemplate(`
        ...
        - name (required)
        - region (optional, must be one of: midwest, bay, tahoe, sac, socal, scranton, uncategorized). If none, default to uncategorized.
        - county (optional, must be a string). If none, default to null.
    ...
    const newPerson = await createPerson({
      ...
      region: getEnumValue(Region, personData.region),

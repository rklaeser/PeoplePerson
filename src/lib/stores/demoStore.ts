...
export const demoActions = {
  ...
  updateGroup(id: string, updates: Partial<DemoGroup>) {
    demoGroups.update(groups =>
      groups.map(group =>
        group.id === id ? updateTimestamp({ ...group, ...updates }) : group
      )
    );
  ...
  createAssociation(personId: string, associateId: string) {
    demoPersonAssociations.update(assocs => {
      if (assocs.some(assoc => assoc.personId === personId && assoc.associateId === associateId)) {
        return assocs; // Already exists
      }
...
export function getDemoPersonWithDetails(id: string) {
  ...
  const unsubscribeGroupAssocs = demoGroupAssociations.subscribe(value => groupAssociations = value);
  const unsubscribePersonAssocs = demoPersonAssociations.subscribe(value => personAssociations = value);
...
export function getDemoGroupWithPeople(id: string) {
  ...
  const availablePeople = allPeople.filter(person => 
    !people.some(groupPerson => groupPerson.id === person.id)
  );

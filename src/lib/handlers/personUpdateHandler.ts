...
export class PersonUpdateHandler {
	static async handle(
		...
		try {
			...
			if (needsClarification && matchedIds.length > 1) {
				...
				const matchedFriends = allFriends.filter((friend) => matchedIds.includes(friend.id));
			...
			if (matchedIds.length === 0) {
				return {
					...
					action: 'error',
					message:
						'Could not find the person you want to update. Please be more specific or create a new friend instead.',
					people: []
				};
			}
	 ...
	 */
	private static async createHistoryEntries(
		oldFriend: Friend,

	...
	$: displayGroups =
		$isDemoMode || data?.isDemo
			? $demoGroups.map((group) => {
					const memberCount = $demoGroupAssociations.filter(
						(assoc) => assoc.groupId === group.id
					).length;
					return {
						...group,
				...
				<div class="divide-y divide-gray-700">
					{#each displayGroups as group}
						<div
							...
							on:keydown={(event) => {
								if (event.key === 'Enter' || event.key === ' ') navigateToGroup(group.id);
							}}
							tabindex="0"
							role="button"
						>

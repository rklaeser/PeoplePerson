<script lang="ts">
	...
	export let friendsData: any[] = []; // Friends data passed from parent (for demo mode)
	...
	// Use passed friends data in demo mode, otherwise use store
	$: actualFriendsData = $isDemoMode ? friendsData : $friends;

	$: filteredPeople = actualFriendsData
		.filter((person) => {
		...
		selectedGroup === ''
			? filteredPeople.reduce(
					(acc, person) => {
						...
						const groupName =
							person.Groups && person.Groups.length > 0 ? person.Groups[0].name : 'No Group';
						if (!acc[groupName]) {
							acc[groupName] = [];
						}
	...
	$: sortedGroupNames = selectedGroup === '' ? Object.keys(groupedPeople).sort() : [''];
</script>
		...
		<div>
			{#each sortedGroupNames as groupName}
				{#if selectedGroup === ''}
					<!-- Group Header -->
					<div
						class="bg-gray-200 px-4 py-2 font-semibold text-gray-700 border-b border-gray-300 sticky top-0"
					>
				...
				{#each groupedPeople[groupName] as person}
					<div
						class="flex gap-x-4 px-4 py-2 border-b border-gray-300 hover:bg-gray-600 cursor-pointer items-center"
						aria-label={`View ${person.name}'s profile`}
						on:click={() => navigateToFriend(person.id)}
						on:keydown={(event) => {
							if (event.key === 'Enter' || event.key === ' ') navigateToFriend(person.id);
						}}
						tabindex="0"
						role="button"
					>
